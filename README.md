...
* **/api/add**: Registra un nuevo nodo en la base de datos interna SQLite (nodos.db). Almacena de forma indexada su uuid, dirección ip, puerto y base_folio.
* **/api/act**: Actualiza los parámetros de red o folios de un nodo existente basándose en su identificador único.
* **/api/del**: Remueve permanentemente a un nodo del directorio activo de señalización.

### Consultas e información (GET).

* **/apishow?basefolio=VALOR**: Retorna la lista de todos los nodos activos vinculados a un lote de folios específico.
...
Nota: Si se está probando localmente sin túnel, puede sustituir la URL por http://localhost:8000.
...
#### Consultar nodos activos por base_folio (/api/show)

Ejecute:

```
curl -X GET "https://abcd1234.serveousercontent.com/api/show?base_folio=FOLIO_B" \
  -H "X-API-Key: mi_api_key_secreta_123456"
```
...
* Los campos uuid y base_folio son textos libres; se recomienda usar identificadores únicos para evitar conflictos.
